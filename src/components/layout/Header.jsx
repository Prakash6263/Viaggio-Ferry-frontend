...
export default function Header() {
  ...
  const { company: sidebarCompany, user: sidebarUser, clearSidebar } = useSidebar()
  const [companyProfile, setCompanyProfile] = useState(null)

  useEffect(() => {
    const fetchProfile = async () => {
      try {
        // FIXED: Check user role before fetching company or user profile
        let response
        if (sidebarUser?.role === "company") {
          response = await loginApi.getCompanyProfile()
        } else {
          response = await loginApi.getUserProfile()
        }
  ...
  }, [navigate, clearSidebar, sidebarUser?.role])
