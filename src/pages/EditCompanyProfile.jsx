...
const EditCompanyProfile = () => {
  ...
  const parseArrayField = (value) => {
    ...
    // If already a string (comma-separated), return as-is
    if (typeof value === "string") {
      // Check if it's a JSON string (starts with [ or contains backslashes)
      if (value.startsWith("[") || value.includes("\\")) {
        try {
          // Handle double-escaped JSON by repeatedly parsing
          let parsed = value
          ...
          // If parsing fails, return the original value (likely already comma-separated)
          return value
        }
      }
      return value
  ...
  const fetchCompanyProfile = async () => {
    try {
      ...
      // Image previews
      setImagePreviews({
        logo: company.logoUrl ? `${API_BASE_URL}${company.logoUrl}` : null,
        whoWeAreImage: company.whoWeAreImage ? `${API_BASE_URL}${company.whoWeAreImage}` : null,
        adminProfileImage: company.adminProfileImage ? `${API_BASE_URL}${company.adminProfileImage}` : null,
      })
      ...
      setFormData({
        ...
        applicableTaxes: parseArrayField(company.applicableTaxes),
        operatingPorts: parseArrayField(company.operatingPorts),
        operatingCountries: parseArrayField(company.operatingCountries),
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      Object.entries(formData).forEach(([key, value]) => {
        if (key === "logo" || key === "whoWeAreImage" || key === "adminProfileImage") return
      ...
      if (formData.whoWeAreImage) fd.append("whoWeAreImage", formData.whoWeAreImage)
      if (formData.adminProfileImage) fd.append("adminProfileImage", formData.adminProfileImage)
