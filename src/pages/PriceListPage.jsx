...
export default function PriceListPage() {
  ...
  useEffect(() => {
    // replicate original DOMContentLoaded tab init: click passenger tab by default
    setActiveTab("passenger");
  }, []);

  // handlers for showDetails/showList (keeps same names as original functions)
  const showDetails = (which) => {
  ...
  const renderTabPane = (tab) => {
    ...
    if (showDetailsFor === tab) {
      return <div id={detailsViewId} className="details-view">{/* details form */}<PriceDetailsForm idPrefix={tab} onBack={showList} /></div>;
    }
    ...
    return (
      <div id={listViewId} className="list-view">
        ...
        <PriceTable onRowClick={() => showDetails(tab)} />
      </div>
    );
  };
